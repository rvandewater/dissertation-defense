...
## Electronic Health Records (EHR): Digital Footprints of the Patient Journey
<div text-align="center">
![](figures/motivation/ehr_simplified.svg){fig-cap="EHR Data Structure" width=100%}
</div>
  <div class="fragment fade-out" data-fragment-index="1"
       style="position:absolute; top:45%; left:0%; width:100%; height:100%; background:white; z-index:2;"></div>
<!-- 
...
- The number of papers on machine learning in health has been growing rapidly, reflecting the increasing interest and potential of this field as shown here from the ArXiv preprint server with ML and health keywords in the title or abstract.
- However, despite this growth in research, the translation of machine learning models into clinical practice has been slow and fraught with challenges.
:::
...
::: {.notes}
- The translation gap in healthcare: despite the promise and increase of AI and machine learning research, new datasets, and advances in methodology that address practical challenges, few models have been successfully deployed in clinical practice.
- **{{< var p1_label >}}**: Retrospective data may not capture the real-time information clinicians use for decision-making, leading to models that perform well in silico but fail in practice.
...
- **{{< var p3_label >}}**: Each institution builds their own custom solutions, making it hard to share research results and build on each other's work.
- **{{< var p4_label >}}**: Proving patient safety and privacy, fairness, and clinical utility to governance bodies is challenging
:::
...
- Development of Applied Multi-ward Multi-modal Early Warning Systems: I conducted the CASSANDRA study, which prospectively collects data from surgical patients in the general ward and integrates wearable device data to improve early warning systems for postoperative complications.
- We developed the YAIB and Recipies framework to enable reproducible benchmarking of ICU prediction models across different datasets and preprocessing choices. We lower the barrier for reproducible research and enable better comparison of models across studies.
- I contributed to the development of the MEDS framework, which provides a standardized data format and tools for scalable machine learning research across healthcare institutions, creating a scalable foundation for open science.
...
## Research Pillars Contributions {auto-animate=true auto-animate-id="pub-flow"}
...
**​⚡​ Retrospective, observational, data does not reflect the information needed for clinical decision-making.**
...
![](icons/surgeon.svg){fig-cap="" height=3em .no-dark}
...
<div class="slide-card slide-card--stack-md">
  <div class="slide-card__row slide-card__row--tight">
![](icons/alert.svg){fig-cap="" height=3em .no-dark}
    <strong>Adverse events impact quality of life and cost^[[@brownImpactPostoperativeComplications2014; @downeyImpactInhospitalPostoperative2023; @kongASOVisualAbstract2021; @muallaImpactPostoperativeComplications2021; @sartelliComplicatedIntraabdominalInfections2014; @vonlanthenImpactComplicationsCosts2011; @wangPostoperativeComplicationsPrognosis2019]]</strong>
  </div>
</div>
:::
:::
...
<div class="slide-card">
  <div class="slide-card__row">
![](icons/hospital.svg){fig-cap="" height=3em .no-dark}
...
<div class="slide-card slide-card--stack-md">
  <div class="slide-card__row">
![](icons/heartbeat.svg){fig-cap="" height=3em .no-dark}
...
::: {.notes}
- Retrospective data may not capture the real-time information clinicians use for decision-making, leading to models that don't capture the full complexity of the patient journey. This is reflected in the case of surgery rehabilition.
...
- But, preceding physiological patterns are often subtle, nonspecific, and time-varying, with substantial heterogeneity between patients^[[@roppeltArtificialIntelligenceHealthcare2024;@stamPredictionSurgicalComplications2022]] → hard to detect on ward, where most postoperative patients rehabilitate.
...
<img src="figures/cass/cashews_logo.svg" width=100% class="no-dark" style="position:absolute; top:70%; left:80%; width:15%; z-index:1;">
...
<div style="text-align: center; margin-top: -0em;">
  <img src="figures/cass/cass_performance.svg" width="80%" class="no-dark">
</div>
</div>
...
- Standard static pre-ward data is not sufficient; dynamic models needed.
- Significant gains from continuous, high-resolution wearable data.
</div>
:::
...
<div style="text-align: center; margin-top: -2em;">
  <img src="figures/cass/shap_contribution.svg" width="100%" class="no-dark">
</div>
::: -->
...
:::
::: {.column width="30%"}
![](figures/cass/shap_contribution.svg){fig-cap="" width=100% .no-dark}
<div style="position: relative;">
- SHAP-values^[[@lundbergUnifiedApproachInterpreting2017a]]: subtle (wearable) physiological indicate early autonomic stress.
</div>
:::
:::
::: {.notes}
- Add either tsne or violin plot shap values here
- 
:::
<!-- 
---
...
::: {.columns}
::: {.column width="70%"}
- **Alarm strategy**: when should we choose to inform clinicians?
...
- Decision curve analysis ^[[@vancalsterReportingInterpretingDecision2018]]: **how much harm (intervention/diagnostics/probes) am I willing to accept?**
<!-- <div style="text-align: center; margin-top: -0em;">
...
Open-access Intensive Care Unit (ICU) data has become popular for applications, models, and benchmarks in ML for health, but → 

::: {.fragment}
​⚡ **The Reproducibility Crisis in Clinical ML**^[[@moorEarlyPredictionSepsis2021;@fleurenMachineLearningPrediction2020a]]:​ Comparing approaches across papers is extremely hard because of:
:::
...
:::
::: {.column width="20%" .fragment}
![](icons/code.svg){fig-align="center"}^[[@shillanUseMachineLearning2019a]]
:::
:::
...
-  **Datasets** (private datasets or different public datasets)
...
- Sepsis prediction^[[@seymourAssessmentClinicalCriteria2016; @moorPredictingSepsisMultisite2021; @calvertComputationalApproachEarly2016]] on MIMIC-IV using Gated Recurrent Unit (GRU) and Light Gradient Boosting Machine (LGBM) models with **different cohort definitions**:
...
# Tools and Pipelines for Foundational Health AI Research 🏗️ {data-section-title="{{< var section_t3 >}}"}
...
<div style="text-align: center; position: absolute; top: 20%;">
  <img src="figures/meds/standards.png" width="100%" class="no-dark"> <a href="https://xkcd.com/927/" target="_blank" style="font-size: 0.5em;">Source: https://xkcd.com/927/</a>
</div>
...
- Tasks and prevalences differ widely across sites → performance comparisons must be contextualized. Each site/dataset independently trains with the same definitions:
:::
::: {.fragment}
...
<div style="text-align: right;  margin-top: -1em;">
  <img src="figures/meds/meds_dev/model_comparison_auroc_task_grouped_samples.svg" width="65%" class="no-dark">
</div>
:::
::: {.column width="50%"}
<div style=" margin-top: -1em;">
  <img src="figures/meds/meds_dev/model_comparison_auprc_task_grouped_samples.svg" width="65%" class="no-dark">
</div>
:::
:::
...
## Linear Probing
- Train SOTA FMs^[[@pangFoMoHClinicallyMeaningful2025]], CEHR-XGPT^[[@pangCEHRXGPTScalableMultiTask2025]] and MOTOR^[[@steinbergMOTORTimetoEventFoundation2023a]], on entire MSHS dataset.
...
TODO: add my own impact (tools, datasets)
...
:::  -->
## Discussion and Impact {auto-animate=true}
...
- Predictive surveillance can be extended into general wards with wearables
...
- Reusable preprocessing pipelines for clinical ML
:::
:::
...
- FMs can be used across datasets and health systems
- MEDS adoption across 27 papers, 17 datasets, and 12 models.
:::
:::
:::
<!-- ::: {.fragment}
...
- Validation strategies and randomized controlled trials are needed to demonstrate clinical utility and safety before deployment 
...
::: {.fragment fragment-index=1}
::: {.card-row .card-row--top}
![](icons/cogs.svg){height=3em .light-only}

**Multi-modal fusion for foundation models: off-the-shelf or health-system specific?**^[[@burkhartFoundationModelsElectronic2025a]] Pathology, radiology, genomics, and whether generalizability-focused optimization is justified.^[[@futomaMythGeneralisabilityClinical2020a; @vancalsterThereNoSuch2023]]
:::
:::
...
::: {.fragment fragment-index=3}
::: {.card-row .card-row--top .card-row--stack}
![](icons/clinician.svg){height=3em .light-only}

**Improving care through predictive surveillance and transferable patient representations.**^[[@kernCareFragmentationCare2024]] Examples: risk-based scheduling (e.g., mammograms) and cross-disciplinary patient representations (e.g., oncology, cardiology).
:::
:::

::: {.notes}
- Different health systems have different data generating processes
:::
