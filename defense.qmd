...
**Central problem: Despite the promise and increase of AI and machine learning research, more abundant data, and advances in methodology, few models have been successfully deployed in clinical practice.** ^[[@apellExplainingSlowAdoption2026] [@henzlerHealthcareProfessionalsPerspectives2025]]
:::
...
- **{{< var p3_label >}}**: Each institution builds their own custom solutions, making it hard to share research results and validate across sites.
...
::: {style="border:1px solid #d9d9d9; border-radius:8px; padding:0.7em 0.9em; background:#f7f7f7;"}
::: {.columns}
::: {.column width="80%"}
...
<div style="border:1px solid #d9d9d9; border-radius:10px; padding:0.7em; min-height:3em; background:#fafafa;">
...
<div style="border:1px solid #d9d9d9; border-radius:10px; padding:0.7em; min-height:3em; background:#fafafa; margin-top:0.6em;">
...
- Physiological patterns are often subtle, nonspecific, and time-varying, with substantial heterogeneity between patients^[[@roppeltArtificialIntelligenceHealthcare2024;@stamPredictionSurgicalComplications2022]] → hard to detect on ward, where most postoperative patients rehabilitate.
...
:::
::: {.fragment .fade-in }
![](icons/cass_timeline.svg){fig-cap="From burden to monitoring gap" width=100%}
:::
::: {.fragment .fade-in}
::: {.column width="80%"}
<div style="position: relative; top: -2em;">
![](figures/cass/wards.png){fig-cap="" width=80% .no-dark}
</div>
:::
...
Corsano band:
- Photoplethysmogram (PPG) Sensor (HR/BP)
- Acceleration
- SPO2
- Respiration Rate
...
![](icons/cass_timeline.svg){fig-cap="From burden to monitoring gap" width=50%}
<div style="position: relative; top: -1.5em;">
![](figures/cass/cass_pipeline_multimodal.svg){fig-cap="" width=100% .no-dark}
</div>
<img src="figures/cass/cashews_logo.svg" width=100% class="no-dark" style="position:absolute; top:80%; left:80%; width:15%; z-index:1;">
...
- Sepsis prediction^[[@seymourAssessmentClinicalCriteria2016; @moorPredictingSepsisMultisite2021; @calvertComputationalApproachEarly2016]] on MIMIC-IV using Gated Reccurent Unit (GRU) and Light Gradient Boosting Machine (LGBM) models:
...
## Generalization Across Datasets: Mortality Prediction 

::: {.column width="50%"}
- Training on row dataset, evaluation on column dataset using GRU:
...
::: {.callout-tip appearance="simple" style="font-size: 1.5em;" .fragment}
How can we enable reproducible ML research across healthcare systems without requiring every institution to engineer their own ecosystem or harmonize their data and lose granularlity?
:::
...
```{=html}
<style>
.card-row p { margin: 0; line-height: 1.2; }
</style>
```

::: {.fragment fragment-index=1}
::: {.card-row style="border:1px solid #d9d9d9; border-radius:8px; background:#fafafa; display:flex; align-items:center; gap:0.4em;"}
![](icons/people_split.svg){height=2em .light-only}
...
::: {.fragment fragment-index=2}
::: {.card-row style="border:1px solid #d9d9d9; border-radius:8px; background:#fafafa; display:flex; align-items:center; gap:0.4em; margin-top:0.3em;"}
![](icons/check.svg){height=2em .light-only}
...
::: {.fragment fragment-index=3}
::: {.card-row style="border:1px solid #d9d9d9; border-radius:8px; background:#fafafa; display:flex; align-items:center; gap:0.4em; margin-top:0.3em;"}
![](icons/wrong.svg){height=2em .light-only}
...
::: {.fragment fragment-index=4}
::: {.card-row style="border:1px solid #d9d9d9; border-radius:8px; background:#fafafa; display:flex; align-items:center; gap:0.4em; margin-top:0.3em;"}
![](icons/thermometer.svg){height=2em .light-only}

**(Large) AI models can have sizeable environmental impact**^[[@hosseiniSocialenvironmentalImpactPerspective2025]]
:::
:::
::: {.notes}
...
::: {.fragment}
- Experimental setup (cohort definitions, preprocessing, evaluation) often matters **more than model choice**
- YAIB provides a flexible, multi-center framework for reproducible ICU benchmarking (44 citations, 96 GitHub stars)
- ReciPies enables modular, reusable preprocessing pipelines for clinical ML
:::
:::
...
::: {.fragment}
- Continuous wearable data **nearly doubles AUPRC** versus models without sensor integration.
- Predictive surveillance can be extended beyond the ICU into general wards with wearable devices.
- Alarm strategies together with decision curve analysis to find the right trade-offs.
:::
:::

::: {.column width="33%"}
**Tools and Pipelines for Foundational Health AI 🏗 ️**

::: {.fragment}

- MEDS-DEV: easily benchmark tasks across different sites and tasks.
- Mount Sinai FMs demonstrate MEDS-based infrastructure enables low-barrier health-system-scale foundation model support.
...
<style>
.card-row p { margin: 0; line-height: 1.2; }
.card-row p:first-child { flex-shrink: 0; }
</style>
```

::: {.fragment fragment-index=1}
::: {.card-row style="border:1px solid #d9d9d9; border-radius:8px; background:#fafafa; display:flex; align-items:flex-start; gap:0.4em;"}
...
::: {.fragment fragment-index=3}
::: {.card-row style="border:1px solid #d9d9d9; border-radius:8px; background:#fafafa; display:flex; align-items:flex-start; gap:0.4em; margin-top:0.3em;"}
