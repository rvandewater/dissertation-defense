...
::: {.fragment .fade-in}
⚡ <span data-id="problem-incomplete-shared" style="display:inline-block; font-weight:700;">$P_1$: Incomplete observations</span> ^[[@agnielBiasesElectronicHealth2018]]
:::
::: {.fragment .fade-in}
⚡ <span data-id="problem-repro-shared" style="display:inline-block; font-weight:700;">$P_2$: Research reproducibility</span>^[[@futomaMythGeneralisabilityClinical2020]]
:::
::: {.fragment .fade-in}
⚡ <span data-id="problem-infrastructure-shared" style="display:inline-block; font-weight:700;">$P_3$: Lack of robust infrastructure</span>^[[@mcdermottLackScienceMachine2025] [@donohoDataScienceSingularity2024]]
:::
::: {.fragment .fade-in}
*Out of scope: ​⚡​ ** $P_4$: Ethical and regulatory hurdles**^[[@vancalsterThereNoSuch2023]] *
:::

::: {.notes}
- **$P_1$: Research reproducibility**: Different papers report different performance for the same task due to variations in datasets, preprocessing, and evaluation.
- **$P_2$: Incomplete observations**: Retrospective data may not capture the real-time information clinicians use for decision-making, leading to models that perform well in silico but fail in practice.
- **$P_3$: Lack of robust infrastructure**: Each institution builds their own custom solutions, making it hard to share research results and validate across sites.
- **$P_4$: Ethical and regulatory hurdles**: Proving patient safety and privacy, fairness, and clinical utility to governance bodies is challenging
:::

## Research Contribution per Challenge {auto-animate=true}


⚡ <span data-id="problem-incomplete-shared" style="display:inline-block; font-weight:700;">$P_1$: Incomplete observations</span>  <span class="fragment fade-in" data-fragment-index="1" data-id="incomplete-phrase">→ $R_1$: Development of Multi-ward Multi-modal Warning Systems 🛏️​</span>

⚡ <span data-id="problem-repro-shared" style="display:inline-block; font-weight:700;">$P_2$: Research reproducibility</span> <span class="fragment fade-in" data-fragment-index="2" data-id="repro-phrase">→ $R_2$: Enabling Reproducible Prediction Experiments 🧪</span>

⚡ <span data-id="problem-infrastructure-shared" style="display:inline-block; font-weight:700;">$P_3$: Lack of robust infrastructure</span>  <span class="fragment fade-in" data-fragment-index="3" data-id="infrastructure-phrase">→ $R_3$: Tools and Pipelines for Foundational Health AI Research 🏗️</span>
...
::: {.column width="33%"}
<span data-id="incomplete-phrase">$R_1$: Development of Multi-ward Multi-modal Early Warning Systems 🛏️​</span>
...
:::
::: {.column width="33%"}
<span data-id="repro-phrase">$R_2$: Enabling Reproducible Prediction Experiments 🧪</span> 
...
::: {.column width="33%"}
<span data-id="infrastructure-phrase">$R_3$: Packages and pipelines for Foundational Health AI Research 🏗️</span>
...
# Enabling Reproducible Prediction Experiments 🧪 {data-section-title="T1: 🧪 Experiment Reproducibility"}
...
- **Code not published, incomplete, not reusable or adaptable.** ^[[@shillanUseMachineLearning2019a]]
:::
...
- Different papers report different performance for the same clinical task due to variations in datasets, cohort definitions, preprocessing steps, and evaluation metrics.
- This makes it difficult to compare approaches across papers and to understand which models or techniques are truly effective.
- The lack of published code, or code that is incomplete, not reusable, or not adaptable, further exacerbates the problem.
- The key question is whether we can create a framework that allows researchers to reliably reproduce scientific results of clinical prediction models across different datasets and preprocessing choices, enabling more meaningful comparisons and accelerating progress in the field.
:::
...
- Sepsis prediction on MIMIC-IV using Gated Reccurent Unit (GRU) and Light Gradient Boosting Machine (LGBM) models:
...
![](figures/yaib/ext_val_mortality_pooled_auprc.svg){fig-cap="External Validation - Mortality AUPRC" width=85% .no-dark}
</div>
:::
...
# Multi-ward Multi-modal Early Warning Systems 🛏️ {data-section-title="T2: 🛏️ Multi-ward Multi-modal Early Warning Systems"}
...
- Physiological patterns are often subtle, nonspecific, and time-varying, with substantial heterogeneity between patients^[[@roppeltArtificialIntelligenceHealthcare2024;@stamPredictionSurgicalComplications2022]] → hard to detect on sparsely monitored general ward, where most postoperative patients rehabilitate.
...
<div style="text-align: center; margin-top: -0em;">
  <img src="figures/cass/cass_performance.svg" width="90%" class="no-dark">
</div>
</div>
...
<div style="text-align: center;">
  <img src="figures/meds/meds_schema.png" width="50%" class="no-dark">
</div>
::: {.tiny-text}
...
::: {.incremental}
- Models may amplify existing (historical) racial, gender, and socioeconomic biases if not carefully validated ^[[@borgesdonascimentoBarriersFacilitatorsUtilizing2023]]
- Validation strategies and randomized controlled trials are needed to demonstrate clinical utility and safety before deployment ^[[@wongExternalValidationWidely2021a]]
- Can be used for harmful purposes (e.g., insurance discrimination, surveillance, etc.)^[[@gaffneyHealthCareUSA2025;@jangOopsInterdisciplinaryStories2025]]
- (Large) AI models can have sizeable environmental impact; simpler is better ^[[@hosseiniSocialenvironmentalImpactPerspective2025]]
:::
